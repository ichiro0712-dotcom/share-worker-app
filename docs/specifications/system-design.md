...
| 指標キー | 指標名 | 定義 | 計算方法 | 使用箇所 |
|----------|--------|------|----------|----------|
| `funnelRegistered` | 登録数（登録動線） | 期間内に新規登録したユーザー数（退会済みユーザーも含む） | users テーブルで期間内の created_at を持つレコード数（deleted_at 不問） | 登録動線分析 |
| `funnelVerified` | メール認証完了数 | 登録ユーザーのうちメール認証を完了したユーザー数 | 登録ユーザーのうち email_verified = true のレコード数 | 登録動線分析 |
| `funnelSearchReached` | 求人検索到達数 | 登録ユーザーのうち求人検索ページに到達したユーザー数 | job_search_page_views テーブルで登録ユーザーの user_id を持つユニーク数 | 登録動線分析 |
| `funnelJobViewed` | 求人詳細閲覧数（登録動線） | 登録ユーザーのうち求人詳細ページを1回以上閲覧したユーザー数 | job_detail_page_views テーブルで登録ユーザーの user_id を持つユニーク数 | 登録動線分析 |
| `funnelBookmarked` | お気に入り登録数（登録動線） | 登録ユーザーのうち1件以上お気に入り登録をしたユーザー数 | bookmarks テーブルで登録ユーザーの user_id を持つユニーク数 | 登録動線分析 |
| `funnelApplied` | 応募完了数（登録動線） | 登録ユーザーのうち1件以上応募したユーザー数 | applications テーブルで登録ユーザーの user_id を持つユニーク数 | 登録動線分析 |
| `overallConversionRate` | 全体コンバージョン率 | 登録ユーザーのうち応募に至ったユーザーの割合 | 応募完了ユーザー数 ÷ 登録ユーザー数 × 100 | 登録動線分析 |
