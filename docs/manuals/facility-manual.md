...
![メッセージ詳細（チャット）画面](./images/facility/07b-message-detail.png)
...
+TASTASでは、以下の3つの方法で通知をお届けします。

| 通知方法 | 説明 | 確認方法 |
|----------|------|----------|
| **アプリ内メッセージ** | チャット画面に自動送信 | メッセージ一覧 |
| **プッシュ通知** | ブラウザに即時通知 | ブラウザ通知 |
| **メール** | 登録アドレスに送信 | メールボックス |

### 9.2 通知一覧（トリガー・タイミング・媒体）

| 通知名 | トリガー | 送信タイミング | メッセージ | Push | メール |
|--------|----------|----------------|:----------:|:----:|:------:|
| 新規応募 | ワーカーが求人に応募 | 即時 | - | ✅ | ✅ |
| マッチング成立 | 応募が承認された時 | 即時 | ✅ | ✅ | ✅ |
| ワーカーキャンセル | マッチング後にワーカーがキャンセル | 即時 | ✅ | ✅ | ✅ |
| 新着メッセージ | ワーカーからメッセージ受信 | 即時 | - | ✅ | - |
| 勤務リマインド | 勤務の前日 | 前日18:00 | ✅ | ✅ | ✅ |
| レビュー依頼 | 勤務が完了した時 | 完了直後 | ✅ | ✅ | - |
| 募集枠充足 | 募集人数に達した時 | 即時 | - | ✅ | - |
| 締切間近 | 応募が少なく締切が近い時 | 締切3日前 | - | ✅ | - |
| オファー受諾 | ワーカーがオファーを受けた時 | 即時 | ✅ | ✅ | ✅ |
| オファー期限切れ | オファーが未応答で期限切れ | 期限到達時 | ✅ | ✅ | - |

### 9.3 通知文章例（施設管理者向け）

以下は施設管理者に届く通知の文章例です。変数部分はダミー値で表示しています。
...
```
【+TASTAS】新しい応募があります
「日勤看護師 急募」に山田 花子さんから応募がありました
```

**メール件名:** 【+TASTAS】求人への新規応募がありました

**メール本文:**
```
特別養護老人ホームさくら 採用ご担当者様

+TASTASをご利用いただきありがとうございます。
...
応募内容の確認・承認は管理画面から行ってください。
https://share-worker-app.vercel.app/admin/applications
...
▼レビューを書く
https://share-worker-app.vercel.app/admin/workers/xxx/review
```
...
**プッシュ通知:**
```
【+TASTAS】オファーが期限切れになりました
山田 花子さんへのオファーが期限切れになりました
```
