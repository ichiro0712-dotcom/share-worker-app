...
このドキュメントは、+タスタス LPからLINE公式アカウントへ誘導する際のパラメータ設定方法を説明します。
...
### 2.3 広告プラットフォーム別URL管理

LINEタグ（広告プラットフォーム別の友だち追加URL）はDB管理され、LP管理画面から動的に追加・編集・削除が可能。

**設定方法:**
1. LP管理画面で「LINEタグ管理」ボタンをクリック
2. モーダルでタグの追加・編集・削除を行う
3. 各タグには以下を設定:
   - **キー**: utm_sourceの値（例: `google`, `meta`, `tiktok`）
   - **表示名**: 管理画面上の表示名（例: `Google広告`）
   - **URL**: LINE友だち追加URL（LIFFのURL等）
4. デフォルトタグを1つ設定（utm_source未指定時に使用）

**広告入稿時のURL:**
```
https://yourdomain.com/lp/1?utm_source={キー}&c={キャンペーンコード}
```
キーはLINEタグ管理で設定した値を使用（例: `google`, `meta`, `tiktok`等）。

**反映タイミング:**
- タグの追加・編集は即座に全LPに反映（LP配信時にDBから動的取得するため）
- LP HTMLファイルの書き換えは不要
