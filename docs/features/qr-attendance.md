...
**アクセス**: `/admin/reports/usage`（サイドメニュー「利用明細」）

#### 3.3.1 機能

| 機能 | 説明 | 実装状況 |
|------|------|---------|
| 年月フィルター | 月単位での絞り込み（前後月移動可能） | ✅ |
| 一覧表示 | カイテク形式の一覧テーブル | ✅ |
| 集計表示 | 件数、給与、交通費、手数料、税額、合計を表示 | ✅ |
| CSV出力 | BOM付きUTF-8でExcel対応（カイテク形式） | ✅ |

#### 3.3.2 表示項目（カイテク仕様）

| 項目 | 説明 |
|------|------|
| 案件ID | 求人ID（jobId）→ リンクで求人詳細ページへ遷移 |
| 日時 | 勤務日と予定時間（例: 2025/10/23 (木)17:00 〜 18:00） |
| ワーカー | ワーカー名 → リンクでワーカー詳細ページへ遷移 |
| 出勤時刻 | 実績出勤時刻（承認された時刻） |
| 退勤時刻 | 実績退勤時刻（承認された時刻） |
| 休憩時間 | 実績休憩時間（分） |
| 給与 | 計算された給与 |
| 交通費 | 交通費 |
| 手数料 | プラットフォーム手数料（給与×30%） |
| 税額 | 消費税（手数料×10%） |
| 合計 | 給与+交通費+手数料+税額 |
...
```csv
案件ID,日時,事業所,ワーカー,出勤時刻,退勤時刻,休憩時間,給与,交通費,手数料,税額,合計
```
...
| 2026-01-15 | PDF資料（タスタス仕様書、カイテクヘルプ）を基に詳細仕様を追記。 |
...
| 2026-01-21 | 利用明細機能（フェーズ4）実装完了。年月フィルター、一覧表示、集計表示、CSV出力機能を追加。 |
| 2026-01-21 | 利用明細をカイテク仕様に準拠。案件ID/手数料/税額を追加、CSV項目をカイテク形式に変更。 |
| 2026-01-21 | 利用明細改善: 応募ID列を削除、日時表示に曜日追加、案件ID/ワーカー名をリンク化。 |
