...
- **面接あり求人**: 応募完了 → 応募中（`APPLIED`） → 施設がマッチング決定 → マッチング成立（`SCHEDULED`）

**募集人数制限**
- 募集人数の上限に達すると、その日の枠は応募不可になる（ただし、面接あり求人は上限を超えて応募可能）
...
**求人オプション**
- ☑️ 面接してからマッチング: 有効にすると、応募後に施設が手動でマッチング決定する
...
| 2025-12-05 | 応募・マッチング機能の再設計（即時マッチング、面接あり求人、募集人数制限、キャンセルルール改訂） |
