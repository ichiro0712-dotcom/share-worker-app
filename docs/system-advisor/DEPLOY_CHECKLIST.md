...
- `advisor_report_drafts.original_request` (Text?, 2026-05-02 ドラフト本体機能)
- `advisor_report_drafts.skeleton_markdown` (Text?, 2026-05-02 ドラフト本体機能)
- `advisor_report_versions.share_token` (VarChar(64), unique, nullable, 2026-05-03 公開シェア機能)
- `advisor_report_versions.shared_at` (Timestamp, nullable, 2026-05-03 公開シェア機能)
- `advisor_report_versions.shared_until` (Timestamp, nullable, 2026-05-04 共有 URL 30 日有効期限)
- `advisor_chat_sessions.bookmarked` (Bool, default false, 2026-05-04 永続保存しおり)
- `advisor_chat_sessions` index 追加: `(bookmarked, updated_at)`

⚠️ **2026-05-04 追加: Vercel Cron 追加が必要** (`vercel.json` を更新済み):
- `/api/cron/advisor-cleanup` を毎日 19:00 UTC (= 04:00 JST) に実行
- 認証: 既存 `ADVISOR_CRON_SECRET` を流用
- 動作: しおりなし Draft/Versions を 30 日後に削除、Audit ログを 90 日 (report_* は 180 日) で削除、失効済み共有 URL 掃除
...
ドラフト本体機能 (2026-05-02 追加) の確認:
- [ ] 「レポート作成」ツールで送信 → 右 Canvas にドラフト要件 + ドラフト本体 (0 埋め表) が表示される
- [ ] チャットで「LP5 まで増やして」と送信 → Canvas のドラフト本体の表が LP5 まで広がる
- [ ] チャットで「会員登録数の表を足して」 → Canvas のドラフト本体に該当表が増える
- [ ] Canvas の「手動編集」ボタン → Markdown 直接編集 → 保存 で反映される
- [ ] レポート要件 (期間 / metric_keys / outline / notes) の手動編集 → 「ドラフト更新」ボタンで保存される
- [ ] 最終的に「レポート作成 (本文生成)」ボタンを押す → ドラフト本体の構造を保ったまま実数値が入る
...
ステージングと同じ 10 個の `CREATE TABLE` + 以下の `ALTER TABLE` (列追加) のみが出ることを確認:
- `advisor_report_drafts.metric_keys` (Json?)
- `advisor_report_drafts.original_request` (Text?)
- `advisor_report_drafts.skeleton_markdown` (Text?)

🚨 既存テーブルに対する `DROP COLUMN` / `DROP TABLE` などの破壊的 SQL があれば即中止。
