...
[Server] reports/generate.ts:
   ...
   5. 結果を AdvisorReportVersion (新バージョン) として保存
   6. status = 'completed'
   ↓
...
| `src/lib/advisor/reports/notify-google-chat.ts` | Google Chat への配信 |
...
- `POST /api/advisor/report/notify-gchat` — Google Chat 配信
