 ...
 * @related knowledge/DESIGN_DECISIONS.md §1.1 (Gemini バイパスの真因)
 * @related knowledge/LATENCY_HISTORY.md §2 Phase B
 *
 * [TOOL:report_create] バイパス処理。
 * Gemini 2.5 Flash 直叩きで 9 秒前後でドラフト初回作成 (要件 + skeleton 一括生成)。
 *
 * 重要:
 * - skeleton 段階で具体的な LP 名 / キャンペーン名 / 求人名は書かない (ANTI_PATTERNS §3)
 * - 「迷ったら更新する側に倒す」原則 (PROMPT_PATTERNS §2)
 * - Gemini 失敗時は Anthropic にフォールバックしない (ANTI_PATTERNS §1)
 */
