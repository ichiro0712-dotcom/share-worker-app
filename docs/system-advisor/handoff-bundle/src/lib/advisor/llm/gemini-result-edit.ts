 ...
 *
 * [TOOL:result_edit] バイパス処理。
 * 生成済みレポート本文の部分修正 → 新バージョン作成。
 *
 * 新規データ取得が必要な指示 (例: '○○の表を追加') の場合、
 * redirect_to_draft=true + draft_instruction を返して
 * orchestrator が裏で auto-redraft (editDraft → upsert → generateReport) を連続実行。
 */
