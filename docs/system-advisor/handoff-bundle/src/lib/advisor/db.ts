/**
 * @spec    docs/07_SECURITY_COST.md (二重防御設計)
 * @related knowledge/DESIGN_DECISIONS.md §4.1
 *
 * 本番 DB への読み取り専用接続 (advisorDataPrisma + runReadOnly)。
 * - Postgres ロール advisor_readonly (SELECT のみ)
 * - アプリ側 SET TRANSACTION READ ONLY
 * - ORM 分離 (Advisor 自身のテーブル用 prisma とは別インスタンス)
 */
