/**
 * @spec    docs/05_SYSTEM_PROMPT.md (Anthropic 用 system prompt)
 * @spec    docs/01_ARCHITECTURE.md (cachedPart + dynamicPart 分離)
 * @related knowledge/LATENCY_HISTORY.md §3 (prompt cache 5 分 ephemeral 戦略)
 * @related knowledge/DESIGN_DECISIONS.md §2.1, §2.2
 *
 * 重要:
 * - cachedPart: METRIC_CATALOG + プロジェクト知識 (5 分 ephemeral cache)
 * - dynamicPart: セッション情報 + 現在のレポートドラフト全体 (毎回変わる)
 * - dynamic にドラフト埋め込みで get_report_draft ツール往復を排除済み
 */
