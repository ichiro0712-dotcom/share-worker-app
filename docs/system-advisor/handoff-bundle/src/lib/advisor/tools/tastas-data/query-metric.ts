/**
 * @spec    docs/03_TOOLS_SPEC.md §3.2 query_metric
 * @related knowledge/BUG_FIX_PLAYBOOK.md #2 (LP 名取得バグ)
 * @related knowledge/BUG_FIX_PLAYBOOK.md #3 (JST 境界処理)
 * @related knowledge/ANTI_PATTERNS.md §3 §4
 *
 * 重要:
 * - groupBy: 'lp_id' は rows[].label に LandingPage.name を JOIN して埋める (resolveLpLabels)
 * - aggregateByDay は take=100k OOM ガード + JST 変換ロジック完全保持
 * - SQL 化は禁止 (JST 境界の数値ズレ事故リスク、ANTI_PATTERNS §4)
 */
