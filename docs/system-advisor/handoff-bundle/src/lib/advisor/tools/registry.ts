/**
 * @spec    docs/03_TOOLS_SPEC.md (19 ツール仕様)
 *
 * 19 ツールを集約 (Core 5 + TASTAS Data 5 + External 5 + Future 2 + Reports 2)。
 * describeAllToolsForLLM() で Anthropic 用 description を生成 +
 * available() でステータス情報を埋め込む (= LLM が未実装ツールを呼ばない判断ができる)。
 */
