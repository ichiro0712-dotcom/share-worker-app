/**
 * @spec    docs/04_REPORT_FEATURE.md (Canvas + レポート機能フロー §3.2)
 * @spec    docs/01_ARCHITECTURE.md (全体構成 §3)
 * @related knowledge/DESIGN_DECISIONS.md §1 (Anthropic + Gemini 二本立て)
 * @related knowledge/LATENCY_HISTORY.md §2 (Phase 別改善履歴)
 *
 * 重要:
 * - L235-257: retire 予定モデルは警告ログのみ。自動置換しない (ANTI_PATTERNS §5)
 * - L344-368: ユーザー入力先頭の [TOOL:xxx] hidden hint で Gemini バイパス分岐
 * - L660-731: audit 記録は metadata のみ (ChatMessage 側に巨大 data 保存)
 *
 * このファイルを編集する前に必ず DESIGN_DECISIONS.md と ANTI_PATTERNS.md を読むこと。
 */
