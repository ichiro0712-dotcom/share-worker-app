...
### STEP 2: 本番 Supabase に読み取り専用ロールを作成 ☐

> Advisor は **本番 Supabase の業務データを「読み取り専用」で参照する** ため、専用ロールを作ります。
> このロールには `SELECT` 権限のみ付与し、`INSERT`/`UPDATE`/`DELETE` は禁止します。
...
| 7 | `ADVISOR_DATA_DATABASE_URL` | STEP 2 で作成した接続文字列 | 本番 Supabase 読み取り専用 |
