...
| 限定求人（勤務済みの方） | 過去にあなたの施設で勤務したワーカーのみが閲覧・応募できます。 |
| 限定求人（お気に入りのみ） | あなたがお気に入り登録しているワーカーのみが閲覧・応募できます。 |
...
4. `requires_interview`は元の値を維持（審査ありなら審査あり、なしならなし）
...
**対応内容**:
- オファーの場合のみ: `requires_interview` を強制的に `false` として処理
- 限定求人: `requires_interview` の値に応じて審査あり/なしを処理
- 即マッチング処理（既存ロジックを使用）
