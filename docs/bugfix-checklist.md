...
| 8 | 必須項目バリデーションUI改善 | ✅ 完了（赤枠・エラーメッセージ） | ✅ | ⬜ |
| 9 | 法人番号の必須化 | ✅ 完了（13桁バリデーション付き） | ✅ | ✅ |
| 10 | 担当者挨拶文のプレースホルダー追加 | ✅ 完了（180文字制限・カウンター付き） | ✅ | ✅ |
| 11 | 求人詳細ページのレイアウト修正 | ✅ 完了（px-4パディング追加） | ✅ | ⬜ |
| 12 | 郵便番号から住所自動入力 | ✅ 完了（AddressSelectorに実装済み） | ✅ | ✅ |
| 13 | 入力フィールドバリデーション強化 | ✅ 完了（カナ自動変換・電話番号ハイフン） | ✅ | ⬜ |
| 14 | 施設管理画面のリンク表示問題 | ✅ 完了（モーダル表示に変更） | ✅ | ⬜ |
| 15 | ワーカー登録時の資格・資格画像必須化 | ✅ 完了（既に実装済み） | ✅ | ⬜ |
| 16 | 性別選択の改善（その他削除、ラベル変更） | ✅ 完了 | ✅ | ⬜ |
...
### #13 入力フィールドバリデーション強化 ✅ 完了

**目的**: 各入力フィールドで適切な入力制限とフォーマット

**対象**:
- [x] カナ: カタカナのみ入力可（ひらがな自動変換機能付き）
- [x] 電話番号: 数字のみ、ハイフン自動挿入（XXX-XXXX-XXXX形式）
- [x] メールアドレス: 適切なフォーマットチェック
- [x] 口座名義（カナ）: スペース許容版カタカナ入力

**実装内容**:
- `utils/inputValidation.ts` に共通バリデーション関数を作成
  - `formatKatakana()` - ひらがな→カタカナ自動変換、非カタカナ除去
  - `formatKatakanaWithSpace()` - スペース許容版（口座名義用）
  - `formatPhoneNumber()` - 数字のみ許可、ハイフン自動挿入
  - `isValidEmail()`, `isValidPhoneNumber()` など

**適用済み画面**:
- [x] `app/register/worker/page.tsx` - ワーカー登録
- [x] `app/mypage/profile/ProfileEditClient.tsx` - プロフィール編集
- [x] `app/admin/facility/page.tsx` - 施設管理
- [x] `app/system-admin/facilities/new/page.tsx` - 新規施設作成
...
- 緊急連絡先を押しても表示されない（API認証エラー）

**原因分析**:
- APIエンドポイントは`admin_session` Cookieをチェック
- しかし施設管理者セッションはlocalStorageに保存
- サーバーサイドではlocalStorageにアクセスできないため認証失敗

**修正内容**:
- ページ遷移からモーダル表示に変更（ユーザー要望を反映）
- `getWorkerDetail`関数に`qualificationCertificates`データを追加
- `app/admin/workers/[id]/page.tsx`にモーダルコンポーネントを追加
  - 資格証明書モーダル: 保有資格一覧 + 証明書画像表示
  - 緊急連絡先モーダル: 氏名・続柄・電話番号・住所表示
- リンクをモーダル表示ボタンに変更
