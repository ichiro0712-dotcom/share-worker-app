...
**マージ実行を指示された場合でも、マージ先が文脈から 100% 明確でない限り、必ずユーザーに確認してから実行すること。**

確認が必要なケース:
- ユーザーが「マージして」とだけ指示し、マージ先が曖昧
- 直前のやり取りから対象 PR が複数候補ある
- feature/fix → develop / develop → main の切り替わりタイミング
- 複数 PR を同時に扱っていて、マージ順序や対象が不明確

確認のフォーマット:
```
マージ先を確認させてください:
- PR #XXX を [develop / main] にマージしますか？
- マージ方式: squash / merge / rebase
```

**過去に誤って main へマージしてしまったインシデントあり。特に main へのマージは厳格に扱う。ユーザーが `develop` と明言していない限り、main へのマージは実行しない。**
