...
import { formatTableId, fetchReferencedTablesAsMarkdown } from '../tools/tastas-data/chat-tables'
...
### 1.5. ❗ ただし「データ取得失敗 / 警告」記述は今回も同じ失敗が続いていない限り削除する
前回バージョンに以下のような **エラー / 警告 / 取得失敗** 記述が含まれていても、
**今回の「収集データ」セクションで対応するツールが正常に値を返している場合は、
これらの警告・注意文を必ず削除して、新しい数値で表を埋め直す**こと。

具体的に削除する対象パターン:
- "> ⚠️ 取得失敗:" や "> ⚠️ データ取得失敗" の callout / blockquote
- 「データ取得に失敗しました」「未指定」「取得できません」「前回レポート時点のデータ」
  「最新の状況ではありません」「(LINE Webhook ... が未実装)」のような注意書き
- 「☒ 未取得」「※ 取得不可」のようなマーカー

判断ルール:
- 今回の「収集データ」セクションでツールが ok (= "取得失敗" メッセージが無い) → **過去の警告を消す**
- 今回も同じツールが失敗している → 警告を残してよい (今回の失敗内容に書き換える)

理由: ユーザーがリトライして数値が取れたのに古い「失敗しました」記述が残ると混乱する。
レポートは「現時点で取れているデータ」を表すべきで、過去の失敗履歴を保持する場所ではない。
...
function buildUserPrompt(input: {
  ...
  /**
   * skeleton / previousResult / originalRequest に含まれる T-XXX 表 ID を解決した
   * Markdown 表データ。Gemini はツールを呼べないので、サーバー側で解決して添付する。
   * 「T-070 の数値で表を埋めて」のような指示が来た時、Gemini がこれを見て
   * skeleton のセルに実際の数値を書き込めるようにする。
   */
  referencedTablesMarkdown?: string | null
}): string {
  const { draft, collected, previousResultMarkdown, referencedTablesMarkdown } = input
  const lines: string[] = []
  ...
  if (previousResultMarkdown && previousResultMarkdown.trim().length > 0) {
    ...
    lines.push('')
    lines.push(
      '❗ **重要 (システムプロンプト §1.5 を厳守)**: 前回バージョンに含まれる ' +
        '「⚠️ 取得失敗」「データ取得に失敗」「前回レポート時点のデータ」「最新の状況ではありません」 ' +
        '等の警告・注意文は、今回の「収集データ」で対応ツールが正常に値を返している場合は ' +
        '**必ず削除して新しい数値で書き換える**こと。リトライで取れたデータと一緒に過去の失敗記述が ' +
        '残ると読み手が混乱する。'
    )
    lines.push('')
    lines.push('```markdown')
  ...
  // ❗ 参照される T-XXX 表の中身 (skeleton や previousResult に登場する ID を解決済み)。
  //    Gemini は get_table 等のツールを呼べないので、サーバー側で解決して直接添付する。
  //    skeleton 内のプレースホルダ "0" / "-" を、ここの表の実数値で書き換えるのが目的。
  if (referencedTablesMarkdown && referencedTablesMarkdown.trim().length > 0) {
    lines.push('# 📋 参照される過去の表 (T-XXX) の中身 (最重要)')
    lines.push('')
    lines.push('以下は skeleton / 前回バージョン / ユーザー要望に登場する過去の表 (T-XXX) の **実データ** です。')
    lines.push('**skeleton に「T-XXX のデータを埋める」「T-XXX を参照」のような記述があれば、')
    lines.push('プレースホルダ (0 / "-" / 「(レポート生成時に自動で埋められます)」等) を')
    lines.push('必ず以下の表の rows から取った実数値で置き換えること**。')
    lines.push('')
    lines.push('skeleton の列構成と T-XXX の列構成が違うときは、列名から対応関係を判断して数値を入れる。')
    lines.push('skeleton には無い T-XXX の列を勝手に追加してはいけない (skeleton の列だけを埋める)。')
    lines.push('合計行 (太字「合計」) があれば本文行を全部足した値を入れる。')
    lines.push('')
    lines.push(referencedTablesMarkdown)
    lines.push('')
  }
...
export async function generateReport(input: GenerateReportInput): Promise<GenerateReportOutput> {
  ...
  if (draft.dataSources.length === 0) {
    // セッション内に既存表 (T-XXX) が 1 件以上ある場合は dataSources 空でも続行を許可する。
    // 理由: 「T-060 の数値で埋めて」のような既存表だけを使うレポートでは、
    //   Gemini が data_sources を空で返すケースがある (DATA_SOURCE_OPTIONS に
    //   chat_table が未登録のため)。レポート骨格は既存表で埋まるので生成可能。
    const sessionTableCount = await prisma.advisorChatTable.count({
    ...
    // chat_table を擬似 dataSource として追加して続行。
    // collect.ts 側で chat_table は実ツール呼び出しをスキップする扱い (既存設計)。
    draft.dataSources = ['chat_table']
    console.log(
      `[advisor:report-generate] dataSources is empty but ${sessionTableCount} session tables exist → continuing with chat_table pseudo source`
    )
  }

  await markDraftGenerating(draft.id)

  // markDraftGenerating 以降の全処理を try/catch で包む。
  // 過去事故: collectReportData / createReportVersion / appendMessage / recordAudit 等で
  // 予期せぬ例外が出ると DB status が 'generating' のまま張り付き、UI が「生成中」を
  // 永久表示するバグがあった (中止ボタンも消える)。
  // → catch 経路で必ず saveDraftError を呼んで status を 'failed' に倒す。
  try {
    ...
    // 1.5. skeleton / previousResult / originalRequest に含まれる T-XXX 表 ID を
    //      解決して中身を Gemini プロンプトに添付する。
    //      Gemini はツール (get_table) を呼べないので、ここで先回りで解決する必要がある。
    //      skeleton に「T-070 のデータを埋める」と書かれているケースで、Gemini が
    //      数値を実際にセルに展開できるようにする。
    const referencedTables = await fetchReferencedTablesAsMarkdown(draft.skeletonMarkdown, {
      extraSources: [previous?.resultMarkdown ?? null, draft.originalRequest ?? null],
      maxRowsPerTable: 200,
    }).catch((e) => {
      console.warn('[advisor:report-generate] referenced tables fetch failed:', e)
      return { markdown: '', foundIds: [] as string[], missingIds: [] as string[] }
    })
    if (referencedTables.foundIds.length > 0) {
      console.log(
        `[advisor:report-generate] referenced tables resolved (${referencedTables.foundIds.join(', ')})`
      )
    }
    ...
    try {
      geminiOut = await generateWithGemini({
        ...
        userPrompt: buildUserPrompt({
          ...
          referencedTablesMarkdown: referencedTables.markdown,
        }),
        model: GEMINI_MODEL,
      ...
      // throw e だと Gemini SDK が plain object を reject した場合に上位で
      // [object Object] になる → 必ず Error クラスでラップして再 throw する
      throw new Error(`Gemini 呼び出し失敗: ${msg}`)
    }

    // 2.5 自動補完 (空き穴を Claude Haiku の Tool Use で埋め直す)。
    ...
    const autoFillFlag = process.env.ADVISOR_AUTO_FILL_ENABLED
    console.log(
      `[advisor:report-generate] auto-fill flag check: ADVISOR_AUTO_FILL_ENABLED="${autoFillFlag}" (enabled=${autoFillFlag === 'true'})`
    )
    if (autoFillFlag === 'true') {
      try {
        const gaps = detectGapBlocks(geminiOut.text)
        console.log(
          `[advisor:report-generate] auto-fill: detected ${gaps.length} gap blocks in ${geminiOut.text.length} char markdown`
        )
        if (gaps.length > 0) {
          const groups = groupGapBlocks(gaps)
          console.log(
            `[advisor:report-generate] auto-fill: ${groups.length} groups, calling autoFillReportGaps ...`
          )

          // セッション内で作られた表 (advisor_chat_tables) の一覧を取得し、
          // auto-fill が `get_table` で再参照できるよう ctx 経由で渡す。
          // ユーザーが「T-060 の数値で埋めて」とドラフトに指示しても、その指示は
          // skeleton_markdown には反映されないため、auto-fill 側に「何が取れるか」を
          // 構造的に教えてやる必要がある。
          let availableTables: Array<{ tableId: string; purpose: string; rowCount: number }> | undefined
          try {
            const tables = await prisma.advisorChatTable.findMany({
              where: { session_id: input.sessionId },
              orderBy: { id: 'asc' },
              select: { id: true, purpose: true, row_count: true },
            })
            if (tables.length > 0) {
              availableTables = tables.map((t) => ({
                tableId: formatTableId(t.id),
                purpose: t.purpose ?? '',
                rowCount: t.row_count,
              }))
              console.log(
                `[advisor:report-generate] auto-fill: ${availableTables.length} session tables available (${availableTables.map((t) => t.tableId).join(', ')})`
              )
            }
          } catch (e) {
            // 取得失敗時は availableTables なしで続行 (auto-fill は従来通りツールで取りに行く)
            console.warn(
              '[advisor:report-generate] failed to fetch session tables for auto-fill:',
          ...
          console.log(
            `[advisor:report-generate] auto-fill: anySuccess=${autoFill.anySuccess} attempts=${autoFill.attempts.length} totalMs=${autoFill.totalMs}`
          )
    ...
    // markDraftGenerating 以降のどこかで例外が出た場合の catch-all。
    // Gemini 個別 catch から再 throw されたものも含む (saveDraftError は冪等)。
    const errAny = e as { name?: string; message?: string } | undefined
    const msg = e instanceof Error ? e.message : String(e)
    const aborted =
    ...
    if (!aborted) {
      await saveDraftError({
        id: draft.id,
        errorMessage: `生成処理失敗: ${msg}`,
      }).catch((saveErr) => {
        console.error('[advisor:report-generate] saveDraftError failed in catch-all', saveErr)
      })
    }
    throw e
  }
