...
// ========================================
// カテゴリ順・項目順は sort_order（配列の並び）で管理。
// 介護施設の7項目は旧ハードコードリストと完全一致させ、既存ワーカーの表示崩れを防ぐ。
...
async function seedExperienceFields() {
  console.log('\n🏥 経験分野マスタを作成中...');
  let categoryCount = 0;
  let fieldCount = 0;
  ...
  for (let ci = 0; ci < experienceFieldMaster.length; ci++) {
    ...
    // カテゴリは name で冪等 upsert（name に unique 制約はないため findFirst → 作成/更新）
    let categoryRecord = await prisma.experienceFieldCategory.findFirst({
      where: { name: category },
    });
    if (categoryRecord) {
      categoryRecord = await prisma.experienceFieldCategory.update({
        where: { id: categoryRecord.id },
        data: { sort_order: ci, is_published: true },
      });
    } else {
      categoryRecord = await prisma.experienceFieldCategory.create({
        data: {
          name: category,
          sort_order: ci,
          is_published: true,
    ...
    for (let fi = 0; fi < fields.length; fi++) {
      ...
      const existing = await prisma.experienceField.findFirst({
        where: { category_id: categoryRecord.id, name },
      });
      if (existing) {
        await prisma.experienceField.update({
          where: { id: existing.id },
          data: { sort_order: fi, is_published: true },
        });
      } else {
  ...
  console.log(`✅ 経験分野マスタを作成しました（新規カテゴリ ${categoryCount}件 / 新規項目 ${fieldCount}件）`);
}
