...
export async function GET(
  ...
  // DBからLP情報を取得
  // utm_sourceがある場合: delivery_lp_number + delivery_utm_source で検索（4LP方式）
  // utm_sourceがない場合: lp_number で直接検索（従来方式）
  let lp;
  if (utmSource) {
    lp = await prisma.landingPage.findFirst({
      where: {
        delivery_lp_number: lpNumber,
        delivery_utm_source: utmSource,
        is_published: true,
      },
    });
  }
  // utm_sourceなし、または一致するLPが見つからない場合はlp_numberで検索
  if (!lp) {
  ...
  // Supabase StorageからファイルをプロキシFetch（LPのlp_numberでStorageパスを構築）
  ...
  try {
    ...
    if (filePath === 'index.html') {
      ...
      // DB上のlp_numberをメタタグとして埋め込む（tracking.jsが正しいLP番号を識別するため）
      const metaTag = `<meta name="lp-number" content="${lp.lp_number}">`;
