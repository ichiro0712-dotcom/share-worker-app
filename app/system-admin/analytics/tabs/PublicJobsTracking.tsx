...
import { ChevronDown, ChevronRight, Filter, ChevronLeft, ArrowUpDown, ArrowUp, ArrowDown, Search } from 'lucide-react';
...
type JobRankingSortKey = 'pv' | 'sessions' | 'ctaClicks' | 'ctr';
...
export default function PublicJobsTracking() {
  // === アクセス状況の状態 ===
  ...
  }, [getAccessDateRange, selectedGenre]);
  ...
  }, [getRankingDateRange, selectedGenre]);
  ...
  }, [rankingMode, selectedGenre]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div>
      {/* 説明 */}
      <p className="text-sm text-slate-500 mb-4">LP0 - /public/jobs の公開求人検索ページのトラッキングデータ</p>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
            >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 text-slate-500">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">PV</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">セッション</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">求人閲覧</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">CTAクリック</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">CTR</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">登録</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">登録率</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">親求人PV</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">親求人セッション</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">応募数</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider">応募率</th>
              ...
              <tbody className="divide-y divide-slate-200">
                ...
                <tr className="bg-indigo-50 font-semibold">
                  ...
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.totalSessions.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.jobDetailPV.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.ctaClicks.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.ctr}%</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.registrations.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.cvr}%</td>
                  <td className="px-4 py-3 text-right text-slate-900">{(accessData.parentJobPV || 0).toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{(accessData.parentJobSessions || 0).toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{(accessData.applicationCount || 0).toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.applicationRate || 0}%</td>
                  <td className="px-4 py-3 text-right text-slate-900">{accessData.avgDaysToApplication || 0}日</td>
                </tr>
                ...
                {accessData.breakdown?.map((row) => (
                  <tr key={row.period} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right text-slate-600">{row.pv.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.sessions.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.jobDetailPV.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.ctaClicks.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.ctr}%</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.registrations.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.cvr}%</td>
                    <td className="px-4 py-3 text-right text-slate-600">{(row.parentJobPV || 0).toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{(row.parentJobSessions || 0).toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{(row.applicationCount || 0).toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.applicationRate || 0}%</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.avgDaysToApplication || 0}日</td>
                  </tr>
                ))}

                {/* キャンペーンコード別行 */}
                {expandedCodes && accessData.campaignBreakdown.map((row) => (
                  <tr key={`camp-${row.campaignCode}`} className="hover:bg-slate-50 bg-amber-50/30">
                    <td className="px-4 py-3 text-slate-700">
                      {row.campaignCode === '(direct)' ? (
                        <span className="text-slate-400 italic">直接アクセス</span>
                      ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 text-slate-500">
                      ...
                      <th
                        className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider cursor-pointer hover:text-slate-700 select-none"
                        onClick={() => handleRankingSort('sessions')}
                      >
                        <span className="inline-flex items-center">セッション<SortIcon sortKey="sessions" /></span>
                      </th>
                      <th
                        className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider cursor-pointer hover:text-slate-700 select-none"
                        onClick={() => handleRankingSort('ctaClicks')}
                      >
                        <span className="inline-flex items-center">CTAクリック<SortIcon sortKey="ctaClicks" /></span>
                      </th>
                      <th
                        className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider cursor-pointer hover:text-slate-700 select-none"
                        onClick={() => handleRankingSort('ctr')}
                      >
                        <span className="inline-flex items-center">CTR<SortIcon sortKey="ctr" /></span>
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200">
                    {sortedJobRanking.map((job, index) => (
                      <tr key={job.jobId} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <a
                              href={`/public/jobs/${job.jobId}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-slate-900 hover:text-indigo-600 hover:underline truncate max-w-[400px]"
                            >
                              {job.jobTitle}
                            </a>
                        ...
                        <td className="px-4 py-3 text-right text-slate-600">{job.sessions.toLocaleString()}</td>
                        <td className="px-4 py-3 text-right text-slate-600">{job.ctaClicks.toLocaleString()}</td>
                        <td className="px-4 py-3 text-right text-slate-600">{job.ctr}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
