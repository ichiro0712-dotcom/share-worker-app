import { getLaborDocumentPreview } from '@/src/lib/actions';
...
export default async function LaborDocumentPreviewPage({ params }: Props) {
  const { id } = await params;
  const data = await getLaborDocumentPreview(parseInt(id));
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white sticky top-0 z-10 border-b border-gray-200">
        <div className="px-4 py-3 flex items-center">
          <Link href={`/jobs/${id}`} className="mr-4">
      ...
      <div className="mx-4 mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-start gap-2">
        ...
        <p className="text-sm text-blue-700">
          これは労働条件通知書のプレビューです。マッチング成立後、あなたの氏名が記載された正式な労働条件通知書が発行されます。
        </p>
      </div>
