...
function ApplicationsContent() {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const jobsData = await getJobsWithApplications(admin.facilityId);
      setJobs(jobsData);
    ...
    <div className="h-full flex flex-col bg-gray-50">
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">求人ごとの応募状況を確認・管理できます</p>
        </div>
                              ...
                              {workDate.applications.map(app => (
                                <div key={app.id} className="bg-white p-3 rounded border border-gray-100 flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    <Link href={`/admin/workers/${app.worker.id}`} className="block relative w-10 h-10 rounded-full overflow-hidden flex-shrink-0 hover:opacity-80">
                                      {app.worker.profileImage ? (
                                        <Image src={app.worker.profileImage} alt={app.worker.name} fill className="object-cover" />
                                      ) : (
                                  ...
                                  <div className="flex items-center gap-2">
                                    {app.status === 'APPLIED' && (
                                      <>
                                        ...
                                        <button
                                          onClick={() => handleStatusUpdate(app.id, 'CANCELLED', 'この応募をキャンセルしますか？')}
                                          disabled={isUpdating === app.id}
                                    ...
                                    {app.status === 'SCHEDULED' && (
                                      <>
                                        ...
                                        <button
                                          onClick={() => handleStatusUpdate(app.id, 'APPLIED', 'マッチングを取り消して応募中に戻しますか？')}
                                          disabled={isUpdating === app.id}
